...
**One session at a time per project.** Codex and Claude Code share the same `.team/` state directory but each session runs its own server instance with an independent in-memory DB; the server warns via `.team/server.lock` when it detects another live instance. That lock stays advisory and warning-only: it does not refuse startup or serialize separate processes. Concurrent sessions no longer corrupt run state — writes use per-process unique temp names with atomic renames, and a lost concurrent state update is detected and logged rather than silently passing — but state still resolves last-writer-wins. Messages persisted by a sibling instance now sync into this instance's feed for runs it already knows about; a run created by a sibling after this instance booted is never picked up. Sequential sessions remain the recommendation. Launch `codex` from the project root — `.team/` resolves from the working directory.
