...
**Hard rule:** in every `team_send_message` call, from MUST be your fixed roster role name — exactly `researcher` — never a task/spawn label. Never use a step-scoped worker name, attempt-suffixed identifier, or filesystem path such as `researcher_step_2_attempt_1` or `/root/workspace/...`; the dashboard attributes messages by roster name only.
