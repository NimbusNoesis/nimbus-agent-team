...
2. **Parse the draft plan**: Count steps, tally the exact declared file strings claimed per step, and identify declared dependencies. Do not normalize paths or infer overlap beyond the exact strings in the plan.
...
5. **Log progress**: Use `team_send_message` at each phase (see Debug Logging; only when a real run ID was supplied — pre-run dispatches skip these messages).
...
Things that could break the plan during execution: dependency cycles, race conditions, missing migrations, breaking API changes, etc. Worktrees are isolation only and never authorize overlapping claims to run concurrently; those claims must serialize.
...
Exact planned file strings claimed by multiple steps without a dependency between them, or files that will likely need changes outside the declared set. Do not normalize paths or infer equivalent strings when checking ownership; every exact-string overlap must serialize, including when worktrees are used.
...
- Does every item in sections 2-6 cite a specific step number or exact planned file string?
- Did I flag every exact-string overlap as serialized, without treating worktrees as permission for concurrent overlap?
