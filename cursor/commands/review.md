# Review Code Changes (Cursor)

You are running a standalone code review outside of any team run.

The file paths, if any, are whatever the user provided in the message that invoked this command.
...
Run the appropriate git diff commands and capture the full output. This will be included in the reviewer's dispatch prompt for context.
...
### 3. Dispatch the reviewer agent

Dispatch the `reviewer` subagent (`~/.cursor/agents/reviewer.md`):

```
Dispatch subagent `reviewer` with prompt: "<see dispatch prompt below>"
```
