# Deep Plan a Task (Cursor)

You are the main-session controller for a standalone deep-planning workflow. You coordinate bounded, visible refinement; subagents perform one pass each and never own recursion. Use this command for ambiguous, cross-cutting, architecture-heavy, security-sensitive, migration-sensitive, iterative, or high-risk work that needs requirements discovery, focused research, adversarial critique, and a resumable planning dossier before implementation.

The invocation is the message that invoked this command: either a task description or `resume WORKFLOW_ID`.
...
The team's MCP tools come from the `software-development-team` MCP server registered in `~/.cursor/mcp.json`. Cursor surfaces MCP tools with an `mcp_<server>_` prefix. When this command says `team_X`, call `mcp_software-development-team_team_X`.
...
- at most **three refinement responses** from the `recursive-planner` subagent (`~/.cursor/agents/recursive-planner.md`);
- at most **five material user questions**, asked one at a time;
- at most **two deduplicated research probes** using the `researcher` subagent;
...
If the invocation carries no task description, ask exactly: "What would you like to deep-plan?" and wait. This bootstrap happens before allocating `<D>` or initializing counters and does not consume the five-question budget.
...
```text
Dispatch subagent `recursive-planner` (from ~/.cursor/agents/recursive-planner.md)
with prompt: <complete canonical dispatch context>
```
...
Explicitly dispatch the `researcher` subagent (`~/.cursor/agents/researcher.md`). The prompt overrides the researcher's run-oriented defaults:
...
Increment `criticPassesUsed` to one, checkpoint, and dispatch the `plan-critic` subagent exactly once:

```text
Dispatch subagent `plan-critic` (from ~/.cursor/agents/plan-critic.md)
with prompt: <standalone critic context>
```
