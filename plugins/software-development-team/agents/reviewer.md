...
3. **Run the COMPLETE verification suite** (see below — this is mandatory, not optional).
...
You MUST run the COMPLETE test suite, not just spot-check. Specifically:
...
If you skip comprehensive verification, you WILL miss regressions. The coder's report that "tests pass" is not sufficient — verify independently.

**Explicitly require comprehensive validation.** Do not take shortcuts by testing only a few scenarios. Run the full suite.

## Git Worktree Review

When the coordinator's dispatch prompt includes a worktree path (e.g., `.worktrees/step-3`), the coder's changes are in that directory on a separate branch. Follow these steps instead of reviewing from the main repo root.

1. **When it applies**: If the dispatch prompt specifies a worktree path, the coder worked entirely within that directory on a branch like `team/{runId}/step-{N}`. All modified files live there.
2. **Reviewing in the worktree**: Read files from the worktree path. For example, if the worktree is at `.worktrees/step-3`, read `.worktrees/step-3/server/src/index.ts` instead of `server/src/index.ts`.
3. **Diff review**: Use `git -C .worktrees/step-3 diff main` (or the base branch) to see exactly what the coder changed. This gives a clear, complete picture of all modifications without reading every file manually.
4. **Running verification in worktree**: cd into the worktree directory before running verification commands. Example:

```bash
cd .worktrees/step-3 && npx vitest run && npx tsc --noEmit
```

5. **Checking commits**: Verify the coder committed their changes to the worktree branch. Run `git -C .worktrees/step-3 log --oneline -5` to check. If there are uncommitted changes in the worktree, flag this as a `needs_revision` issue — the coder must commit before the coordinator can safely merge and clean up.
...
When requesting revision, your summary MUST include for EACH issue:
