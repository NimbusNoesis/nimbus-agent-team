...
> **Running in OpenAI Codex CLI?** The same team runs on Codex too — see [`../../codex/README.md`](../../codex/README.md). The Codex port reuses this plugin's `server/` unchanged; the two hosts coexist. Use them **sequentially** on a given project: each session runs its own server instance against the same `.team/` state (the server logs a warning via `.team/server.lock` when it detects another live instance). Concurrent instances no longer corrupt run state — state writes are atomic with per-process unique temp names, and a lost concurrent update is detected and logged rather than silently passing — and messages persisted by a sibling instance now sync into the dashboard-hosting instance's activity feed. Run state itself still resolves last-writer-wins, so sequential sessions remain the recommendation. Launch sessions from the project root — team state lands in `.team/` relative to the working directory.
